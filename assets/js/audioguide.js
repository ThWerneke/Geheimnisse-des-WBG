...
(function(){
  ...
  ROUTE.stations.forEach(function(st, i){
    ...
    section.innerHTML =
      '<div class="station-sticky">' +
        ...
        '<div class="old-photo-wrap">' +
          ...
          '<div class="scrap s1"><div class="scrap-tex"></div></div>' +
          '<div class="scrap s2"><div class="scrap-tex"></div></div>' +
          '<div class="scrap s3"><div class="scrap-tex"></div></div>' +
        '</div>' +
    ...
    initAudioPlayer(section.querySelector('.audio-player'), st.media.audio);
  ...
  function updateStation(outer){
    ...
    outer.querySelectorAll('.scrap').forEach(function(scrap, i){
      var local = Math.max(0, Math.min(1, (progress - 0.22 - i * 0.09) * 2.2));
      scrap.style.opacity = String(local * (1 - progress * 0.6));
      var dir = (i % 2 === 0) ? -1 : 1;
      scrap.style.transform = 'translate(' + (dir * local * 95).toFixed(1) + 'px,' + (local * 140).toFixed(1) + 'px) rotate(' + (dir * local * 170).toFixed(1) + 'deg)';
    });
  }
